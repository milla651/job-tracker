...
export function KanbanColumn({
  ...
  return (
    <div className="flex flex-col h-full min-w-[300px] w-[300px] rounded-2xl bg-muted/30 border border-border/40 flex-shrink-0">
      {/* Column Header */}
      <div className="p-4 flex items-start justify-between sticky top-0 bg-transparent z-10">
        <div className="flex items-start gap-2 flex-1">
          ...
          <span className="text-xs text-muted-foreground bg-background/50 px-2 py-0.5 rounded-full border border-border/50 flex-shrink-0">
            {jobs.length}
          </span>
        </div>
      ...
      <div
        ...
        className={cn(
          ...
          isArchive
            ? "border-2 border-dashed border-destructive/20 bg-destructive/5 justify-center items-center text-center"
            : "",
        )}>
        {isArchive ? (
          <div className="text-muted-foreground pointer-events-none">
            ...
            <p className="text-xs mt-1 opacity-70">
              Drag in here to archive applications
            </p>
          </div>
        ) : (
          <>
